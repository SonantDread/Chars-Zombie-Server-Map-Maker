...
void onTick(CMovement@ this)
{
	...
	if(getMap().getTimeSinceStart() < 100)
	{
	...
	if (blob.isOnLadder() || blob.hasTag("flying") && !blob.isAttached() && !blob.isOnGround() && !kk)
	{
		...
		blob.AddForce(ladderforce * moveVars.overallScale * (blob.hasTag("flying") ? 250.0f : 100.0f));
		//damp vel
		Vec2f vel = blob.getVelocity();
		vel *= 0.05f;
	...
	if (blob.isKeyPressed(key_up) && moveVars.canVault)
	{
		...
		// boost over corner
		Vec2f groundNormal = blob.getGroundNormal();
		bool onMap =  blob.hasTag("flying") ? false : blob.isOnMap();
	...
	if (moveVars.jumpFactor > 0.01f && !kk)
	{
	...
	{	
		...
		if (blob.isKeyJustPressed(key_up) && blob.getName() == "builder")
			blob.Tag("flying");
	}
	else
	{
	...
	bool left_or_right = (left || right);
	{
...

		{
			...
			if (!kk && ((absx < lim) || left && greater || right && !greater))
			{
	...
	//falling count
	if (!onground && vel.y > 0.1f && !blob.hasTag("flying"))
	{
		moveVars.fallCount++;
	}
	else
